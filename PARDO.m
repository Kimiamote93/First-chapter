...
%       textFiles      : Listing all text files, should be written as for
%                        example: textFiles = dir ('Input\New Bylot\Bylot 2016\MiniDOT_BYL37 2015-2016\7392-041680\*.txt') 
...
if (lake == "Ward Hunt")                       % Ward Hunt lake files are different from others 
    for k = nstart:numfiles
    
    %changing the current folder back to the folder which text files are in
    %it
    cd(directory)
    fid = fopen(textFiles(k).name, 'rt') ;
    %changing the current folder back to where the PARDO function is located
